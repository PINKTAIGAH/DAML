...
def find_mc_truth(truth_df, tracker_df, tracker_hit_idx):
    """
...
def find_truth_momentum(truth_df, track_df, hit_pair_idx, track_momenta,):
    ...
    # Flatten the hit pair and track momenta list since we do not care about event numbers
    hit_pair_idx    = flatten_list(hit_pair_idx)
    track_momenta   = flatten_list(track_momenta)
    ...
    # Loop over all  
    for hit_idx, track_momentum in zip(hit_pair_idx, track_momenta):
        # Find the idx for the matching truth idx
        truth_idx = find_mc_truth(truth_df, track_df, hit_idx[0])
        # Append momenta to lists only if matching truth hit was found
        if truth_idx is not None:
            # Append tracker hit momentum
            hit_momenta.append(track_momentum[-1])
            # Append truth momentum
            truth_momenta.append(truth_df.iloc[truth_idx]["Momentum"])
    
    return np.array(hit_momenta), np.array(truth_momenta)
